"""
BOA Statement Parser - FINAL FIXED VERSION
Fixed: Header detection was too broad and caught transaction descriptions
"""
...
class StatementParser:
    ...
    def parse_bofa(self, text):
        """Parse BOA - FIXED: Better header detection"""
        transactions = []
        lines = text.split('\n')
        ...
        for line in lines:
            ...
            # FIXED: Only skip if line STARTS with Date/Description/Amount (actual header row)
            # Don't skip if these words just appear in transaction description
    ...
    def parse_chase(self, text):
        """Parse Chase statement - handles AMOUNT and BALANCE columns"""
        transactions = []
        lines = text.split('\n')
        current_year = datetime.now().year
        ...
        for line in lines:
            ...
            if 'TRANSACTION DETAIL' in line_upper:
            ...
            if line_upper.startswith('DATE') or line_upper.startswith('DESCRIPTION') or line_upper.startswith('AMOUNT') or line_upper.startswith('BALANCE'):
                continue
            if 'BEGINNING BALANCE' in line_upper or 'ENDING BALANCE' in line_upper:
                continue
            ...
            if date_match:
                ...
                if len(numbers) >= 2:
                    # Second-to-last number is AMOUNT, last number is BALANCE
                ...
                try:
                    if len(description) > 2:
                        ...
                        # Parse amount - check for negative sign
                        amount = float(amount_str)
                        
                        # Chase shows debits as negative
